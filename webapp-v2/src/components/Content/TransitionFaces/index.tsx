...
import { generateTransitions, saveFaceSerie } from "services/api/FaceGeneratorApi"
...
const TransitionFaces: React.FC = () => {
  ...
  const { images: TransitionFacesImages, count: transitionFacesCount } = useRenderImages({
  ...
  const {
    mutate: mutateSaveFaceSerie, isLoading: isLoadingSaveSerie
    ...
    onError: (error) => {
      if (error instanceof ApiError) {
        toastError(`Error saving face serie with id ${transitionFacesSerie!.id!}. ${error.toString()}`);
      }
    }
  });

  const onSaveSerie = () => {
    if (transitionFacesSerie === undefined || transitionFacesSerie!.id === undefined) {
